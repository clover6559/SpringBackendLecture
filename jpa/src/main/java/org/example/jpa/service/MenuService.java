...
import org.example.jpa.MenuResponse;
...
public class MenuService {
  ...
  @Transactional(readOnly = true)
  public List<Menu> findPrice(int min, int max) {
  ...
  public List<MenuResponse> findExpensiveMenusInCategory2(String categoryName, int minPrice) {
    return repository.findByCategoryNameAndPriceGreaterThanEqualOrderByPriceDesc(categoryName, minPrice).stream()
        .map(m -> new MenuResponse(m.getId(), m.getName(), m.getPrice(), m.getCategory().getName()))
        .toList();
  }
  @Transactional(readOnly = true)
