...
public class CompletableFutureExceptionDemo {
    ...
    private static void demonstrateExceptionally() {
        ...
        CompletableFuture<String> paymentFuture = CompletableFuture
                .supplyAsync(()-> {
                    System.out.println(now() + " [payment] 결제 API 호출 시작");
                    sleep(1000);
                    if (true) {
                ...
                .exceptionally(exception -> {
                    System.out.println(now() + " [exceptionally] 예외 복구: " + exception.getMessage());
    ...
    private static void demonstrateHandle() {
        ...
        CompletableFuture<String> resultFuture =
                CompletableFuture.supplyAsync(()-> {
                    System.out.println(now() + " [order] 주문 처리 시작");
                    sleep(1000);
                    return "ORDER-100";
                }, executorService)
                        .handle((result, exception) -> {
                            if (exception != null) {
                                System.out.println(now() + " [handle] 주문 실패 처리");
                                return "주문 실패: 다시 시도해주세요.";
                            }
                            System.out.println(now() + " [handle] 주문 성공 처리: "+ result);
                            return "주문 성공: " + result;
                        });
                            System.out.println(now() + " [main] 최종 결과: "+ resultFuture.join());
        executorService.shutdown();
    }
    ...
    private static void demonstrateWhenComplete() {
        ...
        CompletableFuture<String> deliveryFuture =
                CompletableFuture.supplyAsync(()-> {
                    System.out.println(now() + " [delivery] 배송 정보 생성 시작");
                    sleep(1000);
                    return "DELIVERY-500";
                }, executorService)
                        .whenComplete((result, exception) -> {
                            if (exception != null) {
                                System.out.println(now() + " [whenComplete] 실패 로그 기록: "+ exception.getMessage());
                            } else {
                                System.out.println(now() + " [whenComplete] 성공 로그 기록: "+ result);
                            }
                        });
