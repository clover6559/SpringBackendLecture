...
public class CompletableFutureCombinationDemo {
    ...
    private static void demonstrateThenCompose() {
        ...
        CompletableFuture<String> signupFuture = createUserId(executorService)
                .thenCompose(userId -> createProfile(userId, executorService))
                .thenCompose(profileId -> sendWelcomeEmail(profileId, executorService));
    ...
    private static void demonstrateThenCombine() {
        ...
        CompletableFuture<Integer> priceFuture = CompletableFuture.supplyAsync(()-> {
            System.out.println( now() + " [price] 상품 가격 조회 시작");
            sleep(2000);
            System.out.println(now() + " [price] 상품 가격 조회 완료");
            return 10000;
        }, executorService);

        CompletableFuture<Integer> discountFuture =  CompletableFuture.supplyAsync(() -> {
            System.out.println( now() + " [discount] 할인 금액 조회 시작");
            sleep(1500);
            System.out.println(now() + " [discount] 할인 금액 조회 완료");
            return 2000;
        }, executorService);
        ...
        System.out.println(now() + " [main] 최종 결제 금액: " + finalPrice);
        executorService.shutdown();
    }
