...
public class CompletableFutureBasicDemo {
    ...
    private static void demonstrateRunAsync() {
        ...
        CompletableFuture<Void> logFuture = CompletableFuture.runAsync(() -> {
                    ...
                    System.out.println(now() + " [" + threadName + "] 로그 저장 시작");

                    sleep(1000);
                    System.out.println( now() + " [" + threadName + "] 로그 저장 완료");
    ...
    private static void demonstrateSupplyAsyncAndChaining() {
        ...
        CompletableFuture<Void> future = CompletableFuture
                        ...
                        .supplyAsync(() -> {
                            ...
                            String threadName = Thread.currentThread().getName();
                            System.out.println(now() + " [" + threadName + "] 사용자 정보 조회 시작");

                            sleep(1500);
                            System.out.println(now() + " [" + threadName + "] 사용자 정보 조회 완료");
                            return "user-1";

                        }, userExecutor)

                        .thenApply(username -> {
                            System.out.println(now() + " [thenApply] username 변환");
                            return username.toUpperCase();
                        })

                        .thenAccept(username -> {
                            System.out.println( now() + " [thenAccept] 최종 사용자명 출력: "+ username);
                        })

                        .thenRun(() -> {
                            System.out.println(now() + " [thenRun] 후속 정리 작업 실행");
                        });
