package com.example.javathreadbasicdemo.practice02;
...
public class ThreadCreationDemo {
    ...
    private static void demonstrateThreadInheritance() {
        ...
        CountingTread countingTread = new CountingTread("상속방식", 1,3,500);
        countingTread.start();
        try {
            countingTread.join();
    ...
    private static class CountingTread extends Thread {
        private final String taskName;
        private final int start;
        private final int end;
        private final int delayMillis;

        public CountingTread(String taskName, int start, int end, int delayMillis) {
        ...
        @Override
        public void run() {
            System.out.println("[" + getName() + "] 카운팅 시작");
    ...
    private static class CountingTask implements Runnable {
            ...
            public void run() {
                ...
                System.out.println("[" + threadName + "] "
                        ...
                        + " 카운팅 시작");
                for (int i=start;i<= end;i++) {
                    System.out.println("[" + threadName + "] 현재 값: " + i);
                    try {
                        Thread.sleep(delayMillis);
                    }catch (InterruptedException e) {
                        Thread.currentThread().interrupt();
