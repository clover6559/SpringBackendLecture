package com.example.javathreadbasicdemo.practice06;
...
public class ConcurrentCollectionDemo {
    ...
    private static void runMapTest(
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            threads[i] = new Thread(() -> {
                        for (int j = 0; j < operationCount; j++) {
                            String key = "user-" + (j % 100);

                            map.merge(key,1,Integer::sum);
                        }
                    });
            threads[i].setName(testName + "-Thread-" + i
            );
    }
        ...
        int expectedTotal = threadCount * operationCount;
        int actualTotal = map.values()
                        .stream()
                        .mapToInt(Integer::intValue)
                        .sum();
        System.out.println("예상 총합: "+ expectedTotal);
        System.out.println("실제 총합: "+ actualTotal);
        ...
        Thread producer = new Thread(() -> {
                    try {
                        for (int i = 1; i <= 8; i++) {
                            ...
                            String task = "Task-" + i;
                            System.out.println("[Producer] 작업 생성: " + task);

                            taskQueue.put(task);
                            System.out.println("[Producer] 큐에 추가 완료: " + task + ", 현재 큐 크기: " + taskQueue.size());
                    ...
                    try {
                        while (true) {
                            ...
                            if ("SHUTDOWN".equals(task)) {
                                ...
                                System.out.println("[Consumer] 종료 신호 수신");
                                break;
                            }

                            System.out.println("[Consumer] 작업 처리 시작: " + task);

                            Thread.sleep(1000);

                            System.out.println("[Consumer] 작업 처리 완료: " + task + ", 현재 큐 크기: "  + taskQueue.size());
                        }
