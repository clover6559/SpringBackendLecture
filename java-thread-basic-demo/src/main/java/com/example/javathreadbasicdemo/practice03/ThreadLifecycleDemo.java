    ...
    public static void demonstrateStartSleepJoin() {
        ...
        private static void demonstrateInterrupt() {
            ...
            Thread longRunningThread = new Thread(()-> {
                ...
                try {
                    ...
                    System.out.println("[" + threadName + "] sleep 중 interrupt 신호 감지");
                    Thread.currentThread().interrupt();
                }
        ...
        private static void demonstrateThreadState() {
            ...
            Thread stateThread = new Thread(()-> {
                String threadName = Thread.currentThread().getName();
                try {
                    System.out.println("[" + threadName + "] 실행 시작");
                    Thread.sleep(1500);
                    synchronized (lock) {
                        System.out.println( "[" + threadName + "] wait 상태 진입");
                        lock.wait(1000);
                    }
            ...
            Thread monitorThread = new Thread(()-> {
                try {
                    while (stateThread.getState() != Thread.State.TERMINATED) {
                        System.out.println("[MonitorThread] StateThread 상태: " + stateThread.getState());
                        Thread.sleep(300);
                    }
                    System.out.println("[MonitorThread] StateThread 상태: " + stateThread.getState());
                }catch (InterruptedException e) {
            ...
            System.out.println("stateThread 시작 전 상태: " + stateThread.getState());
            monitorThread.start();
            stateThread.start();
